...
const objConstructor = new Productos(await getData())
const productos = objConstructor.product
...
const pageShoppingCart = () => {
  const subTotalProductsBuy = carrito.reduce((acc, p) => acc + (p.quantity * p.precio), 0)
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-8">
        ...
        ${carrito.length > 0 ? `<p id="buttonDeleteCart"class="button btn btn-dark">Delete Cart</p>` : ""}
      ...
      <div class="col-lg-3 col-lg-offset-1">
        ...
        ${carrito.length > 0 ? `<p id="buttonCheckout"class="button btn btn-dark">Checkout</p>` : ""}
      </div>
    </div>
  </div>
</section>`
  if (carrito.length > 0) {
    ...
    buttonCheckout.addEventListener("click", () => {
      Swal.fire(
        ...
        `The total of your purchase is: ${subTotalProductsBuy - shipping}`,
        'success'
      )
      deleteCart()
    })
...
const generateShoppingCartList = () => {
  ...
  carrito.forEach(p => {
    if (p.quantity > 0) {
      let totalProduct = p.quantity * p.precio
...
const addItemCart = (product, quantity) => {
  ...
  Toastify({
    text: `se ha añadido el producto ${product.nombre} en el carrito`,
    duration: 3000,
...
const deleteItem = (id) => {
  ...
  localStorage.setItem("carrito", JSON.stringify(carrito))
  Toastify({
    text: `se ha quitado el producto ${findItem.nombre} en el carrito`,
    duration: 3000,
    destination: "https://github.com/apvarun/toastify-js",
    newWindow: true,  
    close: true,
